...
namespace fms
{
	...
	std::uint16_t rikeying_chunk::serialize(byte_writer &to)
	{
		...
		// Host order, symmetric with service::get_sid's read of the scramble
		// words: the swap cancels, so a spec peer interoperates either way.
		to << m_rsid;
	...
	std::uint16_t range_ack_chunk::serialize(byte_writer &to)
	{
		...
		{
			to.write_vlu(range.first);
			to.write_vlu(range.second);
		}
