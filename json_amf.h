...
namespace intertalk
{
	...
	namespace detail
	{
		...
		{
			if (val.is_string())
				return boost::make_shared<amf0_string>(val.get<std::string>());
			else if (val.is_boolean())
				return boost::make_shared<amf0_boolean>(val.get<bool>());
			else if (val.is_null())
				return boost::make_shared<amf0_null>();
			else if (val.is_number())
				return boost::make_shared<amf0_number>(val.get<double>());
			else if (val.is_array())
			{
				amf0_strict_array_ptr a = boost::make_shared<amf0_strict_array>();
			...
			{
				amf0_object_ptr o = boost::make_shared<amf0_object>();
	...
	struct json_amf
	{
		...
		static json amf0_to_json(amf0_type_ptr val)
		{
			...
			{
				amf0_boolean_ptr b = boost::dynamic_pointer_cast<amf0_boolean>(val);
				return json(b->value());
			}
			...
			{
				amf0_number_ptr n = boost::dynamic_pointer_cast<amf0_number>(val);
				double d = n->value();
				double t;
			...
			{
				amf0_string_ptr s = boost::dynamic_pointer_cast<amf0_string>(val);
				return json(s->value());
			}
			case amf0_type::eAMF0StrictArray:
			{
				amf0_strict_array_ptr a = boost::dynamic_pointer_cast<amf0_strict_array>(val);
			...
			{
				amf0_ecma_array_ptr a = boost::dynamic_pointer_cast<amf0_ecma_array>(val);
				json jarr = json::array();
			...
			{
				amf0_object_ptr o = boost::dynamic_pointer_cast<amf0_object>(val);
