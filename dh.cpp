...
namespace intertalk
{
	...
	bool dh::copy_key(const BIGNUM *key, boost::uint8_t *output, boost::uint32_t size)
	{
		boost::int32_t s = BN_num_bytes(key);
		if (s <= 0 || s > static_cast<boost::int32_t>(size))
			return false;
	...
	void dh::create_shared_key(boost::uint8_t *key, boost::uint16_t size)
	{
		boost::uint16_t s = DH_size(m_dh);
		m_shared_key = new boost::uint8_t[s];
	...
	void dh::copy_shared_key(boost::uint8_t *key, boost::uint16_t size)
	{
		std::memcpy(key, m_shared_key, size);
	}

	void dh::copy_public_key(boost::uint8_t *key, boost::uint16_t size)
	{
	...
	void dh::copy_private_key(boost::uint8_t *key, boost::uint16_t size)
	{
