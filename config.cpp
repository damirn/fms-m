...
namespace intertalk
{
	...
	void config::create_description()
	{
		m_description.add_options()
			...
			("rtmfp-port,K", boost::program_options::value<boost::uint16_t>(&m_rtmpf_port)->default_value(_eRTMFPDefaultPort), "RTMFP listen port")
			("websocket-port,W", boost::program_options::value<boost::uint16_t>(&m_ws_port)->default_value(_eWSServerPort), "Websocket listen port")
			...
			("local-sip-port,p", boost::program_options::value<boost::uint16_t>(&m_sip_port)->default_value(_eSIPPort), "local SIP port")
			...
			("log-level,l", boost::program_options::value<boost::uint32_t>(&m_log_level)->default_value(_eDefaultLogLevel), "log level")
			...
			("quality,q", boost::program_options::value<boost::uint32_t>(&m_speex_quality)->default_value(_eSpeexQuality), "speex quality")
			("max-audio-frames,e", boost::program_options::value<boost::uint32_t>(&m_max_elements_in_audio_queue)->default_value(_eDefaultElementsInAudioQueue), "max number of enqueued audio frames")
			("max-audio-frames-high-latency,E", boost::program_options::value<boost::uint32_t>(&m_max_elements_in_audio_queue_high)->default_value(_eDefaultElementsInAudioQueueHighLatency), "max number of enqueued audio frames (high latency)")
			("recorded-message-file,M", boost::program_options::value<std::string>(&m_recorded_message_file), "recorded message flv file")
			("threads,t", boost::program_options::value<boost::uint32_t>(&m_threads)->default_value(_eDefaultThreads), "number of I/O threads to use")
			("notify-threshold,n", boost::program_options::value<boost::uint32_t>(&m_notify_delay_threshold)->default_value(_eNotifyThreshold), "notify delay threshold (in ms)")
			("terminate-threshold,r", boost::program_options::value<boost::uint32_t>(&m_terminate_delay_threshold)->default_value(_eTerminateThreshold), "terminate delay threshold")
			...
			("js-server-port,J", boost::program_options::value<boost::uint16_t>(&m_js_server_port)->default_value(_eJSServerPort), "application server port")
			("password-file,F", boost::program_options::value<std::string>(&m_password_file)->default_value("./passwd"), "password file for admin app")
			("admin-data-keep-time,A", boost::program_options::value<boost::uint32_t>(&m_admin_data_keep_time)->default_value(_eAdminDataKeepTime), "keep time for admin data")
